...
def compute_link_DS(l,k):
    """ Computes the linking number between 2 polylines 
    - uses JIT compiling via numba and auto-parallelization for faster execution 
    """
    λ = 0
    Nl = l.shape[0]
    Nk = k.shape[0]
...
class StreamGen:
    """
    Class containing the velocity field at time t, computes vorticity and estimates helicity through streamline linking number computation
    ...
    def integrate_streamline(self, x0):
        ...
        t_bounds = [0, self.T]
        rtol = 1e-3 # test integration parameters
        atol = 1e-4
    ...
    def generate_streamlines_method(self):
        """
        Generates N_sample streamlines - not functional as parallel code
        """
        # samples N_sample random points uniformly inside the ball (Polar sampling method)
        ...
        streamlines = []
        for x in points:
            streamlines.append(self.integrate_streamline(x))
        return streamlines
    ...
    def estimate_helicity(self):
        ...
        for _ in range(self.N_rounds):
            # generates N_sample streamlines
            streamlines = self.generate_streamlines()
            #discretize streamlines by naive segment evaluation.
            discrete_stream = []
            ...
            for i in range(N_sample):
                ...
                for j in range(0, i-1):
                    ...
                    # the factor 2 comes for counting the integral twice for (i,j) and (j,i).
                    # unsure whether it is actually required.
                    self.hel_est /= self.T*self.T
                    hel_estimates_seq.append(self.hel_est) # keeps track of the running value of the helicity estimate
